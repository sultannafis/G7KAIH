...
use App\Http\Controllers\MasterAdmin\SettingsController;

use App\Http\Controllers\UserManagement\TeacherController;
use App\Http\Controllers\UserManagement\StudentController;
use App\Http\Controllers\UserManagement\ParentController;
...
Route::middleware(['auth', 'verified'])->group(function () {
    ...
    // Dashboard
    // ---------------------------------------------------------------
    Route::get('/dashboard/admin',       [AdminDashboardController::class, 'index'])->name('dashboard.admin')->middleware('role:admin');
    Route::get('/dashboard/masteradmin', [MasterAdminDashboardController::class, 'index'])->name('dashboard.masteradmin')->middleware('role:masteradmin');
    Route::get('/dashboard/parent',      [ParentDashboardController::class, 'index'])->name('dashboard.parent')->middleware('role:orangtua');
    Route::get('/dashboard/student',     [StudentDashboardController::class, 'index'])->name('dashboard.student')->middleware('role:siswa');
    Route::get('/dashboard/teacher',     [TeacherDashboardController::class, 'index'])->name('dashboard.teacher')->middleware('role:guru');
    ...
    Route::middleware(['auth', 'role:masteradmin'])->prefix('masteradmin')->name('masteradmin.')->group(function () {
        ...
        Route::patch('notification-templates/{notificationTemplate}/toggle', 
            [NotificationTemplateController::class, 'toggle'])
            ->name('notification-templates.toggle');

        // Settings
        Route::get('settings',                    [SettingsController::class, 'index'])->name('settings.index');
        Route::get('settings/ai',                 [SettingsController::class, 'aiPage'])->name('settings.ai');
        Route::post('settings/ai',                [SettingsController::class, 'aiSave'])->name('settings.ai.save');
        Route::get('settings/notifications',      [SettingsController::class, 'notificationPage'])->name('settings.notifications');
        Route::post('settings/notifications',     [SettingsController::class, 'notificationSave'])->name('settings.notifications.save');
    });

    // =============================================================
    // SCHOOL ADMIN & OTHER ROLES
    // =============================================================

    // AI School Status Check
    Route::get('/ai/school-status', function () {
        $setting = \App\Models\AiSetting::instance();
        return response()->json([
            'can_use_ai' => $setting->schoolCanUseAiValidation(auth()->user()->school_id ?? 0)
        ]);
    })->name('ai.school-status');

    // Notification Templates Access for School Admin
    Route::middleware(['auth', 'role:admin'])->prefix('school-admin')->name('school-admin.')->group(function () {
        Route::resource('notification-templates', NotificationTemplateController::class)->only(['index', 'show', 'edit', 'update']);
        Route::patch('notification-templates/{notificationTemplate}/toggle', [NotificationTemplateController::class, 'toggle'])->name('notification-templates.toggle');
        Route::delete('notification-templates/{notificationTemplate}/fork', [NotificationTemplateController::class, 'destroyFork'])->name('notification-templates.destroy-fork');
    });

    // =============================================================
    // SCHOOL ADMIN MAIN ROUTES
    // =============================================================
    Route::middleware(['auth', 'role:admin'])->prefix('school-admin')->name('school-admin.')->group(function () {
    ...
    // =============================================================
    // UNIFIED USER MANAGEMENT (Admin + Masteradmin)
    // =============================================================
    Route::middleware(['auth', 'role:admin,masteradmin'])->prefix('user-management')->name('user-management.')->group(function () {
    ...
    Route::middleware(['auth', 'role:admin,guru,masteradmin'])->prefix('school-admin')->name('school-admin.')->group(function () {
        ...
        Route::post('habits/{habit}/toggle-status', [HabitController::class, 'toggleStatus'])->name('habits.toggle-status');
        Route::post('habits/{habit}/bulk-generate-prayer-rules', [HabitRuleController::class, 'bulkGeneratePrayerRules'])->name('habits.bulk-generate-prayer-rules');
        Route::resource('habit-items', HabitItemController::class)->parameters(['habit-items' => 'item']);
        Route::post('habit-items/{item}/toggle-status', [HabitItemController::class, 'toggleStatus'])->name('habit-items.toggle-status');
        Route::post('habit-items/{item}/sync-prayer-times', [HabitItemController::class, 'syncPrayerTimes'])->name('habit-items.sync-prayer-times');
        Route::post('habit-items/{habitItem}/generate-prayer-rules', [HabitRuleController::class, 'generatePrayerRules'])->name('habit-items.generate-prayer-rules');
        Route::get('habit-rules/preview-prayer-times', [HabitRuleController::class, 'previewPrayerTimes'])->name('habit-rules.preview-prayer-times');
        Route::post('habit-rules/update-priorities', [HabitRuleController::class, 'updatePriorities'])->name('habit-rules.update-priorities');
        Route::resource('habit-rules', HabitRuleController::class)->parameters(['habit-rules' => 'rule']);
    });
    ...
    Route::middleware(['auth', 'role:siswa'])->prefix('student')->name('student.')->group(function () {
        ...
        Route::get('habits/submission/{submission}',   [HabitSubmissionController::class, 'show'])->name('habits.submission.show');
        Route::get('habits/{habit}/submit',            [HabitSubmissionController::class, 'create'])->name('habits.submit.create');
        Route::post('habits/{habit}/submit',           [HabitSubmissionController::class, 'store'])->name('habits.submit.store');
        Route::post('habits/{habit}/quick-submit',     [HabitSubmissionController::class, 'quickSubmit'])->name('habits.quick-submit');
    });

    // =============================================================
    // PARENT & TEACHER VALIDATION
    // =============================================================
    ...
    Route::middleware(['auth', 'role:guru'])->prefix('teacher')->name('teacher.')->group(function () {
        ...
        Route::get('my-class',                          [MyClassController::class, 'index'])->name('my-class.index');
        Route::get('my-class/bulk-report/download',     [MyClassController::class, 'bulkReportDownload'])->name('my-class.bulk-report-download');
        Route::get('my-class/student/{student}/report', [MyClassController::class, 'studentReport'])->name('my-class.student-report');
        Route::get('my-class/student/{student}/report/download', [MyClassController::class, 'studentReportDownload'])
            ->name('my-class.student-report-download');
    ...
    // Redirect /profile ke halaman sesuai role
