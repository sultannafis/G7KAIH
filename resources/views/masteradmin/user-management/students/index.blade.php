    ...
    <x-slot name="header">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                ...
                <h1 class="text-2xl sm:text-3xl font-bold text-sky-800" style="letter-spacing:-.02em">Semua Data Siswa</h1>
                <p class="text-sky-500 font-medium mt-1 text-sm">Data siswa dari seluruh sekolah</p>
            </div>
        </div>
    </x-slot>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div class="gc fade-in rounded-3xl p-5">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p class="text-2xl font-black text-sky-800">{{ $students->total() }}</p>
                        </div>
                    </div>
                </div>
                {{-- Aktif --}}
                <div class="gc fade-in2 rounded-3xl p-5">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <p class="text-xs font-bold text-emerald-400 uppercase tracking-wider">Siswa Aktif</p>
                            <p class="text-2xl font-black text-sky-800">{{ $students->where('is_active', true)->count() }}</p>
                        </div>
                    </div>
                </div>
                {{-- Non-Aktif --}}
                <div class="gc fade-in3 rounded-3xl p-5">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p class="text-2xl font-black text-sky-800">{{ $students->where('is_active', false)->count() }}</p>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <form method="GET" class="flex flex-wrap gap-3 items-end">
                    <div class="flex-1 min-w-[180px]">
                        ...
                        <input type="text" name="search" value="{{ request('search') }}"
                               class="sky-input" placeholder="Nama, NISN, NIS, email...">
                    </div>
                    ...
                    <div class="flex gap-2">
                        ...
                        @if(request()->hasAny(['search','school_id','grade_level','status']))
                        <a href="{{ route('masteradmin.user-management.students.index') }}"
                            ...
                            <tbody>
                                ...
                                <tr class="trow" style="border-bottom:1px solid rgba(186,230,253,.2)">
                                    ...
                                    <td class="px-5 py-4 hidden sm:table-cell">
                                        <p class="text-sm font-bold text-sky-800 max-w-[160px] truncate">{{ $student->school->name ?? '—' }}</p>
                                        <p class="text-xs text-sky-400 font-medium">{{ $student->school->npsn ?? '' }}</p>
                                    </td>
                                    <td class="px-5 py-4">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div class="min-w-0">
                                                <p class="text-sm font-bold text-sky-800 truncate">{{ $student->name }}</p>
                                                <p class="text-xs text-sky-400 sm:hidden">{{ $student->school->name ?? '' }}</p>
                                                <p class="text-xs text-sky-400 md:hidden">{{ $student->email }}</p>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-5 py-4 hidden md:table-cell">
                                        <span class="text-sm font-medium text-sky-700">{{ $student->email }}</span>
                                    </td>
                                    <td class="px-5 py-4 hidden lg:table-cell">
                                        @if($student->student)
                                            @if($student->student->nisn)
                                                <span class="px-2.5 py-1 rounded-xl text-xs font-bold text-sky-700" style="background:rgba(224,242,254,.7);border:1px solid rgba(186,230,253,.5)">NISN: {{ $student->student->nisn }}</span>
                                            @endif
                                            @if($student->student->nis)
                                                <span class="px-2.5 py-1 rounded-xl text-xs font-bold text-emerald-700 ml-1" style="background:rgba(209,250,229,.7)">NIS: {{ $student->student->nis }}</span>
                                            @endif
                                            @if(!$student->student->nisn && !$student->student->nis)
                                    ...
                                    <td class="px-5 py-4 hidden xl:table-cell">
                                        @if($student->student)
                                            <p class="text-sm font-medium text-sky-800">{{ $student->student->grade_level ?? '' }} {{ $student->student->class_name ?? '' }}</p>
                                            <p class="text-xs text-sky-400">{{ $student->student->major ?? '' }}</p>
                                        @else
                                            <span class="text-sky-300 text-xs">—</span>
                                        @endif
                                    </td>
                                    ...
                                    <td class="px-5 py-4 text-right">
                                        <a href="{{ route('masteradmin.user-management.students.show', $student->student->id ?? '') }}"
                                           title="Lihat Detail"
                ...
                @else
                    <div class="flex flex-col items-center justify-center py-20 px-6 text-center">
                        ...
                        <p class="text-sm text-sky-400">
                            {{ request()->hasAny(['search','school_id','grade_level','status']) ? 'Coba ubah filter pencarian.' : 'Belum ada siswa yang terdaftar di sistem.' }}
                        </p>
                    </div>
                @endif
            </div>
