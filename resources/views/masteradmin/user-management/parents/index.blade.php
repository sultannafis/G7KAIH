    ...
    <x-slot name="header">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                ...
                <h1 class="text-2xl sm:text-3xl font-bold text-sky-800" style="letter-spacing:-.02em">Semua Data Orang Tua/Wali</h1>
                <p class="text-sky-500 font-medium mt-1 text-sm">Data orang tua/wali dari seluruh sekolah</p>
            </div>
        </div>
    </x-slot>
                ...
                <div class="gc fade-in rounded-3xl p-5">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p class="text-2xl font-black text-sky-800">{{ $parents->total() }}</p>
                        </div>
                    </div>
                </div>
                {{-- Aktif --}}
                <div class="gc fade-in2 rounded-3xl p-5">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p class="text-2xl font-black text-sky-800">{{ $parents->where('is_active', true)->count() }}</p>
                        </div>
                    </div>
                </div>
                {{-- Non-Aktif --}}
                <div class="gc fade-in3 rounded-3xl p-5">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p class="text-2xl font-black text-sky-800">{{ $parents->where('is_active', false)->count() }}</p>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <form method="GET" class="flex flex-wrap gap-3 items-end">
                    <div class="flex-1 min-w-[180px]">
                        ...
                        <input type="text" name="search" value="{{ request('search') }}"
                               class="sky-input" placeholder="Nama, email, sekolah...">
                    </div>
                    ...
                    <div class="flex gap-2">
                        ...
                        @if(request()->hasAny(['search','school_id','relationship','status']))
                        <a href="{{ route('masteradmin.user-management.parents.index') }}"
                        ...
                        <table class="min-w-full">
                            <thead>
                                <tr style="border-bottom:1px solid rgba(186,230,253,.4);background:rgba(240,249,255,.5)">
                                    ...
                                    <th class="px-5 py-4 text-left text-xs font-bold text-sky-500 uppercase tracking-wider hidden 2xl:table-cell">Telepon</th>
                            ...
                            <tbody>
                                ...
                                <tr class="trow" style="border-bottom:1px solid rgba(186,230,253,.2)">
                                    ...
                                    <td class="px-5 py-4 hidden sm:table-cell">
                                        <p class="text-sm font-bold text-sky-800 max-w-[160px] truncate">{{ $parent->school->name ?? '—' }}</p>
                                        <p class="text-xs text-sky-400 font-medium">{{ $parent->school->npsn ?? '' }}</p>
                                    </td>
                                    <td class="px-5 py-4">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div class="min-w-0">
                                                <p class="text-sm font-bold text-sky-800 truncate">{{ $parent->name }}</p>
                                                <p class="text-xs text-sky-400 sm:hidden">{{ $parent->school->name ?? '' }}</p>
                                                <p class="text-xs text-sky-400 md:hidden">{{ $parent->email }}</p>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-5 py-4 hidden md:table-cell">
                                        <span class="text-sm font-medium text-sky-700">{{ $parent->email }}</span>
                                    </td>
                                    <td class="px-5 py-4 hidden lg:table-cell">
                                        @if($parent->parent && $parent->parent->family_relationship)
                                            <span class="px-2.5 py-1 rounded-xl text-xs font-bold text-indigo-700" style="background:rgba(224,242,254,.7);border:1px solid rgba(186,230,253,.5)">
                                                {{ $parent->parent->family_relationship }}
                                            </span>
                                        @else
                                    ...
                                    <td class="px-5 py-4 hidden xl:table-cell">
                                        @if($parent->parent && $parent->parent->student)
                                            <p class="text-sm font-medium text-sky-800">{{ $parent->parent->student->user->name ?? '-' }}</p>
                                            <p class="text-xs text-sky-400">{{ $parent->parent->student->nisn ?? '' }}</p>
                                        @else
                                            <span class="text-sky-300 text-xs">—</span>
                                        @endif
                                    </td>
                                    <td class="px-5 py-4 hidden 2xl:table-cell">
                                        <span class="text-sm font-medium text-sky-700">{{ $parent->phone_number ?? '—' }}</span>
                                    </td>
                                    <td class="px-5 py-4">
                                        @if($parent->is_active)
                                    ...
                                    <td class="px-5 py-4 text-right">
                                        <a href="{{ route('masteradmin.user-management.parents.show', $parent->parent->id ?? '') }}"
                                           title="Lihat Detail"
                ...
                @else
                    <div class="flex flex-col items-center justify-center py-20 px-6 text-center">
                        ...
                        <p class="text-sm text-sky-400">
                            {{ request()->hasAny(['search','school_id','relationship','status']) ? 'Coba ubah filter pencarian.' : 'Belum ada orang tua yang terdaftar di sistem.' }}
                        </p>
                    </div>
                @endif
            </div>
