    ...
    <x-slot name="header">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                ...
                <h1 class="text-2xl sm:text-3xl font-bold text-sky-800" style="letter-spacing:-.02em">Manajemen Orang Tua</h1>
                <p class="text-sky-500 font-medium mt-1 text-sm">Kelola data orang tua/wali siswa di sekolah Anda</p>
            </div>
            <div class="flex flex-wrap gap-2">
                <a href="{{ route('school-admin.user-management.parents.create') }}" class="flex items-center gap-2 px-4 py-2 rounded-2xl text-sm font-bold text-white transition-all hover:translate-y-[-1px]" style="background:linear-gradient(135deg,#38bdf8,#0ea5e9);box-shadow:0 4px 12px rgba(14,165,233,.3)">
                ...
                <a href="{{ route('school-admin.user-management.parents.import') }}" class="flex items-center gap-2 px-4 py-2 rounded-2xl text-sm font-bold text-white transition-all hover:translate-y-[-1px]" style="background:linear-gradient(135deg,#34d399,#10b981);box-shadow:0 4px 12px rgba(16,185,129,.3)">
                ...
                <a href="{{ route('school-admin.user-management.parents.download-template') }}" class="flex items-center gap-2 px-4 py-2 rounded-2xl text-sm font-bold text-white transition-all hover:translate-y-[-1px]" style="background:linear-gradient(135deg,#60a5fa,#3b82f6);box-shadow:0 4px 12px rgba(59,130,246,.3)">
    ...
    <div class="pb-10">
        <div class="w-full max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 xl:px-12 space-y-5">
            @php
                ...
                $totalParents   = \App\Models\User::where('school_id',$school->id)->where('role','orang_tua')->count();
                $activeParents  = \App\Models\User::where('school_id',$school->id)->where('role','orang_tua')->where('is_active',true)->count();
                $inactiveParents= \App\Models\User::where('school_id',$school->id)->where('role','orang_tua')->where('is_active',false)->count();
            @endphp
            {{-- Stat Cards --}}
            ...
            <div class="gc-static fade-in4 rounded-3xl p-5">
                <form method="GET" class="flex flex-wrap gap-3 items-end">
                    ...
                    <div class="flex gap-2">
                        ...
                        @if(request()->hasAny(['search','relationship','status']))
                        <a href="{{ route('school-admin.user-management.parents.index') }}"
                                ...
                                <tr class="trow" style="border-bottom:1px solid rgba(186,230,253,.2)">
                                    ...
                                    <td class="px-5 py-4 text-right">
                                        <div class="flex items-center justify-end gap-2">
                                            <a href="{{ route('school-admin.user-management.parents.show', $parent->id ?? '') }}"
                                               title="Lihat Detail"
                                            ...
                                            <a href="{{ route('school-admin.user-management.parents.edit', $parent->id ?? '') }}"
                                               title="Edit"
                                            ...
                                            <form action="{{ route('school-admin.user-management.parents.toggle-status', $parent->id ?? '') }}" method="POST" class="inline"
                                            ...
                                            <form action="{{ route('school-admin.user-management.parents.destroy', $parent->id ?? '') }}" method="POST" class="inline"
                ...
                @else
                    <div class="flex flex-col items-center justify-center py-20 px-6 text-center">
                        ...
                        <p class="text-sm text-sky-400 mb-4">
                            {{ request()->hasAny(['search','relationship','status']) ? 'Coba ubah filter pencarian.' : 'Belum ada orang tua yang terdaftar di sistem.' }}
                        </p>
                        @if(!request()->hasAny(['search','relationship','status']))
                            <a href="{{ route('school-admin.user-management.parents.create') }}" class="flex items-center gap-2 px-6 py-2.5 rounded-2xl text-sm font-bold text-white transition-all hover:translate-y-[-1px]" style="background:linear-gradient(135deg,#38bdf8,#0ea5e9);box-shadow:0 4px 12px rgba(14,165,233,.3)">
                                <svg fill="none" stroke="currentColor" viewBox="0 0 24 24" class="w-4 h-4"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M12 4v16m8-8H4"/></svg>
