    ...
    <x-slot name="header">
        <style>
            ...
            @media (min-width: 768px) and (max-width: 1023px) {
                /* Tablet: show limited columns, hide xl ones */
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                ...
                <h1 class="text-2xl sm:text-3xl font-bold text-sky-800" style="letter-spacing:-.02em">{{ Auth::user()->role === 'masteradmin' ? 'Semua Data Guru' : 'Manajemen Guru' }}</h1>
                <p class="text-sky-500 font-medium mt-1 text-sm">{{ Auth::user()->role === 'masteradmin' ? 'Data guru dari seluruh sekolah' : 'Kelola data tenaga pendidik di sekolah Anda' }}</p>
            </div>
            @if(Auth::user()->role === 'admin')
            <div class="header-actions flex flex-wrap gap-2">
                <a href="{{ route('user-management.teachers.create') }}" class="flex items-center gap-2 px-4 py-2 rounded-2xl text-sm font-bold text-white transition-all hover:translate-y-[-1px]" style="background:linear-gradient(135deg,#38bdf8,#0ea5e9);box-shadow:0 4px 12px rgba(14,165,233,.3)">
                ...
                <a href="{{ route('user-management.teachers.import') }}" class="flex items-center gap-2 px-4 py-2 rounded-2xl text-sm font-bold text-white transition-all hover:translate-y-[-1px]" style="background:linear-gradient(135deg,#34d399,#10b981);box-shadow:0 4px 12px rgba(16,185,129,.3)">
                ...
                <a href="{{ route('user-management.teachers.download-template') }}" class="flex items-center gap-2 px-4 py-2 rounded-2xl text-sm font-bold text-white transition-all hover:translate-y-[-1px]" style="background:linear-gradient(135deg,#60a5fa,#3b82f6);box-shadow:0 4px 12px rgba(59,130,246,.3)">
        ...
        <div class="w-full max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 xl:px-12 space-y-5">
            @php
                if(Auth::user()->role === 'masteradmin') {
                    ...
                    $activeTeachers  = \App\Models\User::where('role','guru')->where('is_active',true)->count();
                    $inactiveTeachers= \App\Models\User::where('role','guru')->where('is_active',false)->count();
                } else {
                    $school = Auth::user()->school;
                    $totalTeachers   = \App\Models\User::where('school_id',$school->id)->where('role','guru')->count();
                    $activeTeachers  = \App\Models\User::where('school_id',$school->id)->where('role','guru')->where('is_active',true)->count();
                    $inactiveTeachers= \App\Models\User::where('school_id',$school->id)->where('role','guru')->where('is_active',false)->count();
                }
            @endphp
                ...
                <div class="gc fade-in rounded-3xl p-5">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p class="text-2xl font-black text-sky-800">{{ $totalTeachers }}</p>
                        </div>
                    </div>
                </div>
                {{-- Aktif --}}
                <div class="gc fade-in2 rounded-3xl p-5">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p class="text-2xl font-black text-sky-800">{{ $activeTeachers }}</p>
                        </div>
                    </div>
                </div>
                {{-- Non-Aktif --}}
                <div class="gc fade-in3 rounded-3xl p-5">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p class="text-2xl font-black text-sky-800">{{ $inactiveTeachers }}</p>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <form method="GET" class="filter-form flex flex-wrap gap-3 items-end">
                    ...
                    <div class="flex-1 min-w-[180px]">
                        ...
                        <input type="text" name="search" value="{{ request('search') }}"
                               class="sky-input" placeholder="Nama, email, NIP, NIK...">
                    </div>
                ...
                @if($teachers->count() > 0)
                    <div class="px-4 sm:px-5 py-3 flex flex-wrap items-center justify-between gap-3" style="background:rgba(240,249,255,.4)">
                        <div class="flex items-center gap-3">
                            <span class="text-xs text-sky-400 font-semibold">Menampilkan {{ $teachers->firstItem() ?? 0 }}–{{ $teachers->lastItem() ?? 0 }} dari {{ $teachers->total() }} guru</span>
                        </div>
                            ...
                            <tbody>
                                ...
                                <tr class="trow" style="border-bottom:1px solid rgba(186,230,253,.2)">
                                    ...
                                    <td class="px-5 py-4 hidden lg:table-cell">
                                        @if($teacher)
                                            @if($teacher->nip)
                                                <span class="px-2.5 py-1 rounded-xl text-xs font-bold text-sky-700 block mb-1 w-max" style="background:rgba(224,242,254,.7);border:1px solid rgba(186,230,253,.5)">NIP: {{ $teacher->nip }}</span>
                                            @endif
                                            @if($teacher->nik)
                                                <span class="px-2.5 py-1 rounded-xl text-xs font-bold text-emerald-700 block w-max" style="background:rgba(209,250,229,.7)">NIK: {{ $teacher->nik }}</span>
                                            @endif
                                            @if(!$teacher->nip && !$teacher->nik)
                                                <span class="text-sky-300 text-xs">—</span>
                                            @endif
                                        @endif
                                    </td>
                                    <td class="px-5 py-4 hidden xl:table-cell">
                                        <span class="text-sm font-medium text-sky-700">{{ $user->phone_number ?? '—' }}</span>
                                    </td>
                                    <td class="px-5 py-4">
                                        @if($user->is_active)
                                    ...
                                    <td class="px-5 py-4 text-right">
                                        <div class="flex items-center justify-end gap-2">
                                            <a href="{{ route('user-management.teachers.show', $teacher->id ?? '') }}"
                                               title="Lihat Detail"
                                            ...
                                            <form action="{{ route('user-management.teachers.toggle-status', $teacher->id ?? '') }}" method="POST" class="inline"
                                                  onsubmit="return confirm('{{ $user->is_active ? 'Nonaktifkan' : 'Aktifkan' }} guru ini?')">
                                                @csrf
                                                <button type="submit"
                                            ...
                                            <form action="{{ route('user-management.teachers.destroy', $teacher->id ?? '') }}" method="POST" class="inline"
                                                  onsubmit="return confirm('Hapus guru ini? Data tidak dapat dikembalikan.')">
                                                @csrf @method('DELETE')
                                                <button type="submit"
                        ...
                        <div class="mobile-card">
                            ...
                            <div class="space-y-1.5 mb-3">
                                ...
                                <div class="flex flex-wrap gap-1.5">
                                    @if($teacher->nip)
                                        <span class="px-2.5 py-1 rounded-xl text-xs font-bold text-sky-700" style="background:rgba(224,242,254,.7);border:1px solid rgba(186,230,253,.5)">NIP: {{ $teacher->nip }}</span>
                                    @endif
                                    @if($teacher->nik)
                                        <span class="px-2.5 py-1 rounded-xl text-xs font-bold text-emerald-700" style="background:rgba(209,250,229,.7)">NIK: {{ $teacher->nik }}</span>
                                    @endif
                                </div>
                                @endif
                                @if($user->phone_number)
                                <div class="flex items-center gap-2">
                                    <span class="sky-label mb-0 shrink-0">Telepon</span>
                                    <span class="text-xs font-semibold text-sky-700">{{ $user->phone_number }}</span>
                                </div>
                                @endif
                            </div>
                ...
                @else
                    <div class="flex flex-col items-center justify-center py-20 px-6 text-center">
                        ...
                        @if(Auth::user()->role === 'admin' && !request()->hasAny(['search','status']))
                            <a href="{{ route('user-management.teachers.create') }}" class="flex items-center gap-2 px-6 py-2.5 rounded-2xl text-sm font-bold text-white transition-all hover:translate-y-[-1px]" style="background:linear-gradient(135deg,#38bdf8,#0ea5e9);box-shadow:0 4px 12px rgba(14,165,233,.3)">
                                <svg fill="none" stroke="currentColor" viewBox="0 0 24 24" class="w-4 h-4"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M12 4v16m8-8H4"/></svg>
                                Tambah Guru Pertama
                            </a>
                        @endif
